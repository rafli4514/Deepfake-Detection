...
async def list_history(skip: int = 0, limit: int = 10):
    db = get_database()
    cursor = db.history.find().skip(skip).limit(limit)
    return await cursor.to_list(length=limit)

@router.get("/{history_id}", response_model=HistoryResponse)
async def get_history(history_id: str):
    db = get_database()
    history = await db.history.find_one({"_id": ObjectId(history_id)})
    if not history:
...
async def delete_history(history_id: str):
    db = get_database()
    delete_result = await db.history.delete_one({"_id": ObjectId(history_id)})
